...
In contrast, [Long-Term Storage](./lts/index.md) is best-suited for data that changes infrequently or never changes. Specifically:
...
Shared Storage is allocated on a per-organization basis, not on a per-person basis or per-project basis. If an individual researcher manages both a Lab and a Core, they may request independent storage allocations for each organization. Every organization may request both Project Storage and Long-Term Storage.
